...
```mermaid
graph TD
    A[Mandi/Stall Default] -->|Biases| B[Dirty/Inconvenient BUT Fresh]
    C[Online D2C Apps Default] -->|Biases| D[Convenient BUT Stale/Frozen]
    E[Relifish Target] -->|Pillars| F[Hyperlocal Trust + Dock-to-Door Freshness]
```
...
```mermaid
graph TD
    A[Identify Society Influencer] -->|Appoint| B[Relifish Captain]
    B -->|Coordinate| C[Weekly Society Group Order]
    C -->|Deliver| D[Single Drop-off Hub]
    D -->|Reward| E[Free Seafood + Credits for Captain]
    E -->|Referrals| F[Viral Building Density]
```
