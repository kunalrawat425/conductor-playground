...
```mermaid
graph TD
    A[Raw Fish Product] -->|Outcome Frame| B[Healthy Family Dinners]
    A -->|Outcome Frame| C[Freshness Without Mandi Visits]
    A -->|Outcome Frame| D[Safe Seafood for Children]
    A -->|Outcome Frame| E[Time Saved Every Week]
```
...
```mermaid
graph LR
    A[Phase 0: Build Waitlist] --> B[Phase 1: Density Threshold]
    B --> C[Phase 2: Launch Article & Ads]
    C --> D[Phase 3: Referral Loop]
```
